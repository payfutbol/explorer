...
class blockchairExplorer{
   ...
   * @returns {Promise.<void>}
   */
  async getRawBlocks(hashOrHeight){
    if(!config.blockchairURL.btc_chain.includes(this.symbol) && !config.blockchairURL.eth_chain.includes(this.symbol)){
      throw notice.createErrorString("getRawBlocks", "The function does not support this chain");
    }
    try{
      var response = await this.api.get("/raw/blocks/"+hashOrHeight+(this.apikey ? "?key="+ this.apikey : ''));
    }catch(e){
      throw notice.createErrorString("getRawBlocks", e)
    }
    return response.data;
  }
   ...
   * @returns {Promise.<void>}
   */
  async getAddress(address, contract){
    if(!config.blockchairURL.btc_chain.includes(this.symbol) && !config.blockchairURL.eth_chain.includes(this.symbol)){
  ...
  async getProperty(prorerty_id){
    ...
    try{
      var response = await this.api.get("/omni/dashboards/property/"+prorerty_id+(this.apikey ? "?key="+ this.apikey : ''));
    }catch(e){
  ...
  async getPropertyList(){
    ...
    try{
      var response = await this.api.get("/omni/property"+(this.apikey ? "?key="+ this.apikey : ''));
    }catch(e){
      throw notice.createErrorString("getProperty", e)
    }
    return response.data;
  ...
  async getNode(){
    ...
    this.api.defaults.baseURL = sprintf(config.blockchairURL.baseURL, "")
    try{
      var response = await this.api.get("/node"+(this.apikey ? "?key="+ this.apikey : ''));
    }catch(e){
